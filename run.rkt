...
(define black-color (make-object color% 255 255 255 1.0))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;   Dialogs ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(define back_simulation_button (new button% [label "Back"]
                                    ...
                                    [callback (lambda (button event) (begin (stop-now)
                                                                            (if square-cell (set! matrix-initial (make-hash))
                                                                                (set! matrix-initial (hex-matrix)))
                                                                            (send alev refresher)
                                                                            ;(send alev refresh)
...
(define (simulator i)
  (cond [(> i n) (display "Done")]
        [else (begin (if square-cell
                         (begin (when (null? (hash-keys matrix-initial)) (set! n 0)) 
  ...
  (if square-cell
      (begin (if (= rows 160)
                 (begin (send bm-dc clear)
                        ...
                        (send bm-dc draw-rectangle xi yi (+ 800 xi) (+ 640 yi))
             ...
             (map (lambda(i)(map (lambda(j)
                                   (when (and (> j -1) (> i -1) (< j cols) (< i rows))
                                     (begin
                                       (send alev increment)
                                       (send bm-dc draw-rectangle (+ (* j size) xi 1) (+ (* i size) yi 1) (- size 1) (- size 1))))) 
        ...
        (map (lambda (l1) (map (lambda (x) (begin
                                             ...
                                             (if (= (remainder (point-i x) 2) 0)
                                                 (let* ([x1  (+ (* (point-j x) (* 3 size)) 30)]
                                                   ...
                                                   (send bm-dc draw-path (draw-hex (+ x1 1) (+ y1 1) size)))
                                                 (let* ([x1  (+ (* (point-j x) (* 3 size)) (* 1.5 size) 30)]
                                                        [y1  (+ (* (point-i x) (* (/ (sqrt 3) 2) size)) 30)])
                                                   (send bm-dc draw-path (draw-hex (+ x1 1) (+ y1 1) size)))))) l1))
    ...
    (define/public refresh
      (λ()
        (begin
          ...
          (when (not (= 0 alive-yet)) (send m change-max alive))
          (set! alive 0)
          ;(send chill set-value (~a alive))
...
(define max%
  (class object%
    ...
    (define/public (change-max m)
      (if (and (not (= 0 m)) (> m max)) (begin (set! max m) ;(display m)
                                               (send max-box set-value (~a max)));;;call for printing new max
          (void)))))
